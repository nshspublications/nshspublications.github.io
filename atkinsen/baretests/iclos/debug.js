...
class BackButton {
  static log = [];
  static logevent(){
    BackButton.log.push(Date.now());
  }
  static dbg_repushstate(){
    history.pushState(null, document.title, location.href);
  }
}

window.addEventListener('popstate', function (event) {
    // 1. Execute your custom "button" action here
    //myCustomButtonAction();
    //console.log("interception");//debug
    //BackButton.log.push(Date.now()); //i guess? for lack of a better reason? 
    BackButton.logevent();

    // 2. Push the dummy state right back so the next back-click is also intercepted
    history.pushState(null, document.title, location.href);
});
...
class dbg_bfgfxt {
  ...
  static frame(){
    ...
    switch(this.pane){
      ...
      default:
        this.pane = "top";
        return "Werid error: switch(pane\n";
    }
    //console.log("molymolymolymoyl");
    if(BackButton.log.length !== 0){
      this.exit();
    }
  }
  static dotop(){
    if(!this.run){
      ...
      { let w = Graphics.width*this.zones.top.pframe.width; 
        let h = Graphics.height*this.zones.top.pframe.height;
        Graphics.rect([0, 0], [w, h]);}
    }
    Graphics.stroke([180,180,220]);
    for(let i=0;i<this.zones.top.rules.length;i++){
      Graphics.line([0, Graphics.height*this.zones.top.rules[i]], [Graphics.width, Graphics.height*this.zones.top.rules[i]]);
    }


    for(const k in Keyboard.keybuffer){
      //console.log(k);
      switch(Keyboard.keybuffer[k]){
        case "Escape":
          this.exit();
      }
    }
  ...
  static exit(){
    Teletype.set_hide_pop();
    dbg_cmd.pop_fd();
  }
  static open(){
    Teletype.set_hide_push(true);
    // dbg_cmd.framediverter = "EXTERN_C";
    // dbg_cmd.framediverterbuffer = ["dbg_bfgfxt", "frame", []];
    dbg_cmd.push_fd("EXTERN_C", ["dbg_bfgfxt", "frame", []]);
    this.reset();
    BackButton.log = [];
  }
}
...
class dbg_cmd {
  ...
  static frame(){
    ...
    //console.log("isit?");
    if(k_framemgr.SIGINT){
      console.log("well, we're here.");
      alert(k_framemgr.SIGINT);
      Console.write("Exited from execution frame: "+JSON.stringify([this.framediverter,this.framediverterbuffer])+"\n");
      //Teletype.out_sw(["","^C"+this.pop_fd(),""]);
      k_framemgr.SIGINT = false;
    }
  ...
  static SIGNAL(code){
    switch(code){
      case 1:
        //SIGINT
        //Console.write("SIGINT");
        Console.write("^C Interrupt; clearing framediverter: "+JSON.stringify([this.framediverter,this.framediverterbuffer])+" with remaining stack: \n"+JSON.stringify(this.fdstack)+"\n");
        Console.write("Set Teletype visibility to VISIBLE (hidden = false); stack: \n"+JSON.stringify(Teletype.hide_stack)+"\n");
        Teletype.set_hide(false);
        this.framediverter = null;
        return 0;
    }
  }
