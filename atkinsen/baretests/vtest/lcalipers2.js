class Calipers2 {
    ...
    static PromptStream = class {
        ...
        Update(keybuffer = Keyboard.keybuffer, keybuffer_backlog = Keyboard.keybuffer_backlog){
            let befcuroff = this.curoff;
            this.file = this.file.slice(0,this.file.length+befcuroff) + Keyboard.flush((str) => Calipers2.Tools.StdKeyHandler((spec) => this.kbfcallback(spec), str), keybuffer, keybuffer_backlog) + this.file.slice(this.file.length+befcuroff,this.file.length);
    ...
    static Keyboard = class {
        ...
        static flush(KeyHandler, keybuffer = this.keybuffer, keybuffer_backlog = this.keybuffer_backlog){ //ex: str += Keyboard.flush((s) => LCTools.StdKeyHandler((specstr) => "", s));
            let out = "";
            keybuffer_backlog = [];
            for(let i=0;i<keybuffer.length;i++){
                out += KeyHandler(keybuffer[i]);
            }
            keybuffer = [...keybuffer_backlog];
            return out;
        }
        static stdflush(){
            let out = "";
